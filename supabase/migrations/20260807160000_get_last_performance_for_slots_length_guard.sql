-- #463 / PR #464 — defence in depth for get_last_performance_for_slots.
--
-- Multi-arg unnest pads shorter arrays with NULLs (it does not truncate). A
-- length mismatch therefore yields half-null slot rows and silent wrong joins.
-- The client already checks via requireParallelSlotArrays; enforce the same
-- contract in SQL because the RPC is granted to authenticated (direct PostgREST).
...
BEGIN
  IF cardinality(p_workout_exercise_ids) IS DISTINCT FROM cardinality(p_exercise_ids) THEN
    RAISE EXCEPTION
      'get_last_performance_for_slots: parallel arrays length mismatch (% vs %)',
      cardinality(p_workout_exercise_ids),
