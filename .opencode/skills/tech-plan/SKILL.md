...
- **i18n contract** — if the epic adds or changes user-facing copy, follow `.opencode/skills/microcopy/SKILL.md` and include the key table (namespace, key, EN, FR, why). Do not write "add i18n keys" as a vague leftover. If there are no new strings, say so in one line.
