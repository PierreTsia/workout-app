...
Enforcement lives in the **ready-signal validator** (per-flow, **server-side, code-level**): a ready signal without a valid `motivation` field is rejected at the Edge — the handler treats the rejection as `ready: false` and continues the conversation (asks the model another turn), it does NOT bubble an error to the UI. The validator is the gate; the system prompt is the instruction. We trust the model with the classification — if post-launch analytics show high "other" rates or visible misclassification, that's the signal to add an eval suite for motivation classification.

**Validator-rejection observability** (added after grilling-session pushback): every rejection fires a new `embedded_agent_motivation_classification_failed` event AND increments a `validator_rejection_count` on the thread row. This separates two failure modes in analytics: (a) the model genuinely abandoned classification and picked `other` to escape rejection (high `validator_rejection_count` on `committed` rows with `motivation: 'other'`), versus (b) the user genuinely has no specific reason (`validator_rejection_count: 0` with `motivation: 'other'`). Without this split, `other` rate is noisy and the post-launch eval-suite trigger (Story 31) is harder to interpret.
