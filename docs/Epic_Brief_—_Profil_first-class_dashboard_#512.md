...
10. As an **athlete**, I want **Mix** stacked by session count (bar height = séances, not a 100% share) with **Mix slice** precedence: **Benchmark Circuit** on the workout day (`benchmark_circuit_id`) > **Quick Workout** (`program_id` null) > **Programme**. Jetable Circuits never take the Circuits slice.
