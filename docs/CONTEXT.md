...
A **named, reusable, catalog-level circuit** — its own entity (`benchmark_circuits`), not a promoted **Exercise Block**. Public handle: immutable `slug` (GymLogic seed `cindy`); PK remains uuid. Forks have `slug` NULL in v1. Hero example: **Cindy** (`AMRAP 20 min`, 5 pull-ups / 10 push-ups / 15 squats) — the WOD people search because of Tom Holland; **Zeus** is the **Tours** twin. Findable later in the **Circuit Catalog** UI (not v1). Droppable into any session, **fixed prescription by definition**. Dropping one onto a **workout day** mints a day-scoped **Exercise Block**: Rx is a **snapshot copy**, plus `benchmark_circuit_id` (nullable — jetable blocks stay unlinked). Score is mode-aware: **Tours** → **Circuit Completion Time**; **AMRAP** → `27+3`. Identity is the catalog id, not the per-day `block_id`; that is why QW AI minting a fresh Cindy-shaped block every time is the anti-pattern. **Resolve, don't mint:** MCP / **Quick Workout AI** / any "Cindy" or alias intent (`holland`, …) loads the seed by `slug` / `aliases`, copies **its** Rx, stamps `benchmark_circuit_id`. The LLM's reconstructed 5-10-15 is discarded — the catalogue wins. A generic Circuit with no seed name stays a jetable **Exercise Block**. Comparability / PR uses **`templateFingerprint`** (`file:src/lib/blockTemplate.ts` — mode + cap + catalog `exercise_id` + amounts/weights), matched against the catalog row's canonical Rx. History / PR for a seed reads `block_runs.benchmark_circuit_id` (GO snapshot), not the live FK on `exercise_blocks`. **Rx storage:** canonical prescription is JSONB on `benchmark_circuits` (mode, cap, `[{ exercise_id, amount, weight }]` — the shape `templateFingerprint` already consumes). Instantiation copies into `block_exercises`. No sibling catalog-exercise table in v1. **Seed immutability:** GymLogic Rx is frozen. Metadata (label, editorial copy, emoji) may be patched. A different Rx is a **different** catalog row, not an in-place version. **Story (not a leaderboard):** each seed carries localized `tagline` + short `story` (2–3 sentences, punchy — not an encyclopedia) and optional `reference` (`{ name, score }`, e.g. Tom Holland / 27). Shown on the #398 history identity surface (later: #393 / shelf). Never mixed into the PR sparkline as a fake run. **Tours** `runFingerprint` is the wrong key (it embeds `block_exercise_id`, unique per copy). Breaking the Rx contract (instance fingerprint ≠ canonical) is a **Circuit Fork**: v1 mints a new user-owned **Benchmark Circuit** (`owner_id` = the athlete) whose Rx is the mutated template, with `forked_from` pointing at the source. The day's **Exercise Block** retargets to the new catalog id. The source row is untouched. Logging leftover / fewer reps than prescribed is *performance*, not a fork — the contract is the template, not the actuals. **Ownership / mutability:** `owner_id uuid NULL` (`NULL` = GymLogic-curated seed). You cannot overwrite a **Benchmark Circuit** you do not own — that is a **Circuit Fork**. A private row you own is mutated **in place** (same id, PRs stay). **V1 population:** GymLogic seeds (`owner_id` NULL) plus **Circuit Fork**. No blank "create a named circuit from scratch" — that is T144, superseded by **Do Cindy** (#393). `visibility` / publish / user-authored-from-scratch come later. **Achievements:** out of #398. No new `metric_type` / Cindy badge. The door is `benchmark_circuit_id` remaining joinable from history so a later RPC can count runs or PRs without a rewrite. Consumption slice: **Do Cindy** (#393) is the home CTA and ad-hoc lifecycle — out of this epic's UI. **#398 v1** ships the noun: seed, instantiate-by-id (photocopie + FK), MCP/QW resolve, **Circuit Fork**, personal PR / history keyed by catalog id. No home button, no shelf, no merge into `/library`. Near-term object of #398; the north star is the **Circuit Catalog**. Not built in #396 / #475.
...
**Circuit Catalog** (planned — north star of #398, not v1):
A library of **Benchmark Circuits** (and, later, shareable full workout days) that is **public and/or user-shareable**, **ranked** on the mode-aware score, with **Achievements** keyed to the catalog identity (e.g. Cindy 20+, beat your last Cindy). Distinct from today's **Library** (`/library`) — that surface is *your* programs plus the exercise catalog, not a social WOD shelf. **No Circuit Catalog UI in #398 v1.** GymLogic-curated presets (Cindy first; Zeus later) seed it; user-published Rx, blank named authoring, and live leaderboards come later. **V1 constraint:** the identity shape shipped for Cindy must survive becoming public / ranked / badged — no private-only seed table that we throw away. V1 population is seeds + **Circuit Fork** only.
