...
**`embedded_workout` quota source**:
**V1** new value in `AIGenerationSource` (`file:supabase/functions/_shared/aiQuota.ts`), fired by the new **Quick Workout AI (v1)** Edge Function on each LLM call. Independent counter from `program` / `embedded_chat` / `embedded_draft` / legacy `workout` — Quick Workout's daily cadence must not compete with onboarding's once-per-user budget. **Cap (v1):** same as today's `workout` source — 5/30days regular, 5/24h whitelisted; revisit with analytics post-launch. **External MCP clients calling `create_workout_day` directly do not burn this quota** (they pay their own LLM tokens).

**`generate-quick-workout` Edge Function**:
**V1** new Supabase Edge surface that backs **Quick Workout AI (v1)**. **Two-phase, Embedded-Agent-shaped** (see **Embedded Agent**) because `PreviewStep` allows the user to edit the AI's suggestion before committing — mirrors the **Onboarding program commit gate** pattern. **Phase 1 (preview):** server runs quota check (**`embedded_workout`**), catalog/profile/history fetch, one-shot Gemini call with workout-specific prompt, validate-and-repair, returns `{ exercises[], rationale }` — **no database write**. **Phase 2 (commit, AI Start only):** accepts the post-edit `{ name, exercises[] }` payload, calls **MCP `create_workout_day`** server → MCP via **MCP Edge Function URL** with the user's session JWT as Bearer, `dry_run: false`. Returns `{ workout_day_id }`. No quota burn on commit (the LLM call already paid). May be one Edge function with two modes or two separate functions — Tech Plan call. Replaces `file:supabase/functions/generate-workout/` (deleted as part of this migration). **AI Save-as-draft does not flow through this function** — drafts stay PWA-local via `useCreateQuickWorkout`.